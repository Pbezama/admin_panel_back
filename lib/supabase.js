...
// ============================================
// STORAGE - CHAT LIVE MEDIA
// ============================================

export async function subirArchivoChatLive(buffer, infoArchivo, idMarca) {
  try {
    const timestamp = Date.now()
    const nombreLimpio = infoArchivo.name.replace(/[^a-zA-Z0-9.-]/g, '_')
    const path = `live-chat/${idMarca}/${timestamp}_${nombreLimpio}`
    ...
    console.error('Error en subirArchivoChatLive:', error)
    return { success: false, error: error.message }
  }
}
...
export async function buscarFlujoPorTrigger(idMarca, canal, mensaje) {
  try {
    ...
    if (error) throw error
    console.log(`[buscarFlujoPorTrigger] idMarca=${idMarca}, canal=${canal} → encontrados: ${data?.length || 0} flujos`, data?.map(f => ({ id: f.id, nombre: f.nombre, canales: f.canales, trigger_tipo: f.trigger_tipo })))
...
export async function actualizarConversacionFlujo(id, datos) {
  try {
    ...
    if (datos.agente_activo_id !== undefined) campos.agente_activo_id = datos.agente_activo_id
...
// ============================================
// LOGS DE FLUJO (MONITOR)
// ============================================

/**
 * Obtener logs de ejecución de una conversación de flujo
 */
export async function obtenerLogsFlujo(conversacionId) {
  try {
    const { data, error } = await supabase
      .from('logs_flujo')
      .select('*')
    ...
    console.error('Error en obtenerLogsFlujo:', error)
    return { success: false, error: error.message }
  }
}

/**
 * Obtener conversaciones de flujo con filtros para el monitor
 */
export async function obtenerConversacionesMonitor(idMarca, filtros = {}) {
  try {
    let query = supabase
      .from('conversaciones_flujo')
      .select('*, flujos(id, nombre, nodos, edges)')
      .eq('id_marca', idMarca)
      .order('creado_en', { ascending: false })
      .limit(filtros.limite || 50)

    if (filtros.flujo_id) query = query.eq('flujo_id', filtros.flujo_id)
    ...
    console.error('Error en obtenerConversacionesMonitor:', error)
    return { success: false, error: error.message }
  }
}
...
/**
 * Obtiene el sitio web de la marca desde cuentas_instagram
 */
export async function obtenerSitioWebMarca(idMarca) {
  try {
    // Buscar usuarios de esta marca
    ...
    if (!usuarios || usuarios.length === 0) {
      return { success: true, sitio_web: null }
    }

    const userIds = usuarios.map(u => u.id.toString())

    // Buscar cuenta de Instagram activa de estos usuarios
    const { data: cuenta, error: igErr } = await supabase
      .from('cuentas_instagram')
      .select('"Sitio web", instagram_name')
      .in('user_id', userIds)
      .eq('activo', true)
      .limit(1)
      .single()

    if (igErr && igErr.code !== 'PGRST116') throw igErr

    return {
      success: true,
      sitio_web: cuenta?.['Sitio web'] || null,
      instagram_name: cuenta?.instagram_name || null
    }
  } catch (error) {
    console.error('Error en obtenerSitioWebMarca:', error)
    return { success: false, error: error.message, sitio_web: null }
  }
}
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        titulo: datos.titulo,
        descripcion: datos.descripcion || null,
        nombre_actor: datos.nombre_actor || null,
...
/**
 * Crear notificación para el creador de una tarea
 * No notifica si el actor es el mismo creador
 */
export async function crearNotificacionTarea(idTarea, tipo, titulo, descripcion, actor) {
  try {
    // Obtener la tarea para saber quién la creó
    const { data: tarea, error: tareaErr } = await supabase
      .from('tareas')
      .select('creado_por, id_marca, titulo')
      .eq('id', idTarea)
      .single()

    if (tareaErr || !tarea) {
      console.error('No se encontró tarea para notificar:', idTarea)
      return { success: false, error: 'Tarea no encontrada' }
    }

    // No notificar si el actor es el creador
    if (tarea.creado_por && actor?.id && tarea.creado_por === actor.id) {
      return { success: true, skipped: true }
    }

    if (!tarea.creado_por) {
      return { success: false, error: 'Tarea sin creador asignado' }
    }

    return await crearNotificacion({
      id_usuario: tarea.creado_por,
      id_tarea: idTarea,
      tipo,
      titulo,
      descripcion,
      nombre_actor: actor?.nombre || 'Sistema',
      id_marca: tarea.id_marca || null
    })
  } catch (error) {
    console.error('Error en crearNotificacionTarea:', error)
    return { success: false, error: error.message }
  }
}
 ...
 */
export async function obtenerNotificacionesUsuario(idUsuario, soloNoLeidas = true) {
  try {
    let query = supabase
      .from('notificaciones')
      .select('*')
...
/**
 * Crear notificacion de transferencia de cliente para todos los ejecutivos de la marca
 */
export async function crearNotificacionTransferencia({ idMarca, canal, identificadorUsuario, nombreCliente, conversacionId }) {
  try {
    // Obtener todos los admins y colaboradores activos de la marca
    ...
    if (usrErr || !usuarios || usuarios.length === 0) {
      console.warn('[Notificacion] Sin usuarios para notificar en marca:', idMarca)
      return { success: false, error: 'Sin usuarios para notificar' }
    }

    const canalLabel = canal === 'web' ? 'Chat Web' : canal === 'whatsapp' ? 'WhatsApp' : canal === 'instagram' ? 'Instagram' : canal
    const clienteLabel = nombreCliente || identificadorUsuario || 'Cliente'

    // Crear notificacion para cada usuario
    const promesas = usuarios.map(u =>
      supabase
        ...
        .insert({
          ...
          titulo: `Nuevo cliente transferido desde ${canalLabel}`,
          descripcion: `${clienteLabel} necesita atencion. Conversacion #${conversacionId}`,
          nombre_actor: 'Sistema de Flujos',
          id_marca: idMarca
        })
    )

    await Promise.allSettled(promesas)
    console.log(`   🔔 Notificacion de transferencia enviada a ${usuarios.length} ejecutivos`)
    return { success: true, notificados: usuarios.length }
  } catch (error) {
    console.error('Error en crearNotificacionTransferencia:', error)
    return { success: false, error: error.message }
  }
}
...
/**
 * Obtener configuracion webchat de una marca (para admin panel)
 */
export async function obtenerWebChatConfigAdmin(idMarca) {
  try {
    const { data, error } = await supabase
      .from('webchat_config')
      .select('*')
    ...
    console.error('Error en obtenerWebChatConfigAdmin:', error)
    return { success: false, error: error.message }
  }
}

/**
 * Crear configuracion webchat para una marca (genera api_key automatico)
 */
export async function crearWebChatConfig(idMarca, datos = {}) {
  try {
    const { data, error } = await supabase
      .from('webchat_config')
      .insert([{
        id_marca: idMarca,
        color_primario: datos.color_primario || '#2d3a5c',
        color_texto_header: datos.color_texto_header || '#ffffff',
        logo_url: datos.logo_url || null,
        posicion: datos.posicion || 'bottom-right',
        tamano: datos.tamano || 'normal',
        titulo_chat: datos.titulo_chat || 'Chat',
        mensaje_bienvenida: datos.mensaje_bienvenida || null,
        horario_activo: datos.horario_activo || false,
        horario_inicio: datos.horario_inicio || null,
        horario_fin: datos.horario_fin || null,
        horario_zona: datos.horario_zona || 'America/Santiago',
        mensaje_fuera_horario: datos.mensaje_fuera_horario || 'Estamos fuera de horario. Te responderemos pronto.'
      }])
      .select()
      .single()
    ...
    console.error('Error en crearWebChatConfig:', error)
    return { success: false, error: error.message }
  }
}

/**
 * Actualizar configuracion webchat de una marca
 */
export async function actualizarWebChatConfig(idMarca, datos) {
  try {
    const camposPermitidos = [
      'activo', 'color_primario', 'color_texto_header', 'logo_url',
      'posicion', 'tamano', 'titulo_chat', 'mensaje_bienvenida',
      'horario_activo', 'horario_inicio', 'horario_fin', 'horario_zona',
    ...
    console.error('Error en actualizarWebChatConfig:', error)
    return { success: false, error: error.message }
  }
}

/**
 * Regenerar API key del webchat
 */
export async function regenerarWebChatKey(idMarca) {
  try {
    const { data, error } = await supabase
      .rpc('gen_random_uuid')

    // Fallback: generar UUID en JS si la funcion RPC no existe
    const nuevoKey = data || crypto.randomUUID()
    ...
    console.error('Error en regenerarWebChatKey:', error)
    return { success: false, error: error.message }
  }
}

/**
 * Obtener configuracion webchat por API key (para el widget publico)
 */
export async function obtenerWebChatConfigPorKey(apiKey) {
  try {
    const { data, error } = await supabase
      .from('webchat_config')
      .select('id_marca, color_primario, color_texto_header, logo_url, posicion, tamano, titulo_chat, mensaje_bienvenida, horario_activo, horario_inicio, horario_fin, horario_zona, mensaje_fuera_horario')
      .eq('api_key', apiKey)
      .eq('activo', true)
      .limit(1)
      .single()
    ...
    if (!data) {
      return { success: false, error: 'API key invalida o widget desactivado' }
    }
    ...
    return {
      ...
      data: {
        ...data,
        nombre_marca: marca?.nombre_marca || 'Chat'
      }
    }
  } catch (error) {
    console.error('Error en obtenerWebChatConfigPorKey:', error)
    return { success: false, error: error.message }
  }
}

/**
 * Obtener id_marca por API key (para validar mensajes)
 */
export async function obtenerMarcaPorWebChatKey(apiKey) {
  try {
    // Usar fetch directo a PostgREST para obtener id_marca como texto
    // y evitar que JavaScript pierda precision con BigInt (>2^53)
    const url = `${process.env.SUPABASE_URL}/rest/v1/webchat_config?select=id_marca&api_key=eq.${apiKey}&activo=eq.true&limit=1`
    ...
    // CSV format: "id_marca\n17841403725357385" - segunda linea es el valor exacto
    const lines = csv.trim().split('\n')
    if (lines.length < 2 || !lines[1]) {
      return { success: false, id_marca: null }
    }
    const idMarcaStr = lines[1].replace(/"/g, '').trim()
    return { success: true, id_marca: idMarcaStr }
  } catch (error) {
    console.error('Error en obtenerMarcaPorWebChatKey:', error)
    return { success: false, id_marca: null }
  }
}

// =====================================================
// CHAT ACADEMICO - Configuracion chatbot PreUCV
// =====================================================

const HERRAMIENTAS_SEMILLA = [
  { nombre: 'obtenerCertificadoAlumnoRegular', nombre_display: 'Certificado Alumno Regular', tipo: 'respuesta_fija', descripcion: 'Proporciona informacion para obtener el certificado de alumno regular.', respuesta_texto: 'Para obtener el certificado de alumno regular, debes solicitarlo mediante correo electronico a docencia@pre-ucv.cl.', parametros_openai: { type: 'object', properties: { consulta: { type: 'string', description: 'Texto de la consulta del cliente.' } }, required: ['consulta'], additionalProperties: false }, orden: 1 },
  { nombre: 'tutorialAgendamiento', nombre_display: 'Tutorial Agendamiento', tipo: 'respuesta_fija', descripcion: 'Proporciona un tutorial de agendamiento para alumnos o apoderados que no sepan como agendar o consulten sobre el horario de clases.', respuesta_texto: 'El sistema de agendamiento se actualiza semana tras semana a traves de la plataforma virtual. Para aprender a agendar tus clases, revisa los siguientes tutoriales Agendamiento Clases Presenciales: [https://youtu.be/rgXn5wPQt-c] Agendamiento Clases Virtuales: [https://youtu.be/UtYQDpVLYIo]', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 2 },
  { nombre: 'comoEntrarAclases', nombre_display: 'Como Entrar a Clases', tipo: 'respuesta_fija', descripcion: 'Proporciona un tutorial para que los alumnos o apoderados aprendan como ingresar a sus clases agendadas y muestra donde esta ubicado el link de las clases.', respuesta_texto: 'Para ingresar a tus clases agendadas, debes acceder a traves del campus virtual. Revisa el siguiente tutorial: [enlace tutorial]', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 3 },
  { nombre: 'solicitudTerminoDeContrato', nombre_display: 'Termino de Contrato', tipo: 'respuesta_fija', descripcion: 'Proporciona formulario google para que los alumnos o apoderados soliciten dar de baja el contrato.', respuesta_texto: 'Para solicitar el termino de contrato, debes completar el siguiente formulario: [https://docs.google.com/forms/d/e/1FAIpQLSdT4tNLZXpjIn1G3a6GnG00DXiyoREWbGA512Ee4lCg2eFp2Q/viewform]', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 4 },
  { nombre: 'derivarConsultaAHumano', nombre_display: 'Derivar a Humano', tipo: 'custom_python', descripcion: 'Deriva la consulta a un agente humano cuando no se puede resolver el problema.', respuesta_texto: 'Lo siento, no he podido resolver tu consulta. Estoy derivando tu caso a uno de nuestro equipo para que te asista.', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 5 },
  { nombre: 'obtenerDatosAlumnoDesdeSheets', nombre_display: 'Buscar Alumno por Telefono', tipo: 'google_sheets', descripcion: 'Obtiene datos del alumno o apoderado a partir de un numero de telefono consultando Google Sheets.', respuesta_texto: '', parametros_openai: { type: 'object', properties: { telefono: { type: 'string', description: 'Numero de telefono para buscar en las hojas de calculo.' } }, required: ['telefono'], additionalProperties: false }, orden: 6 },
  { nombre: 'obtenerDatosAlumnoPorRutDesdeSheets', nombre_display: 'Buscar Alumno por RUT', tipo: 'google_sheets', descripcion: 'Obtiene datos del alumno o apoderado a partir del RUT consultando Google Sheets. Se utiliza cuando no se encuentran datos utilizando el numero de celular.', respuesta_texto: '', parametros_openai: { type: 'object', properties: { rut: { type: 'string', description: 'RUT del alumno para buscar en las hojas de calculo.' } }, required: ['rut'], additionalProperties: false }, orden: 7 },
  { nombre: 'informacionActividades', nombre_display: 'Info Actividades', tipo: 'respuesta_fija', descripcion: 'Proporciona informacion sobre las actividades que le corresponde realizar a un alumno o apoderado.', respuesta_texto: 'Todas las actividades que le corresponde realizar se envian mediante correo electronico la semana anterior a las actividades y mediante grupo de WhatsApp del programa contratado.', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 8 },
  { nombre: 'entrarAlCampus', nombre_display: 'Entrar al Campus', tipo: 'respuesta_fija', descripcion: 'Proporciona informacion para cuando el alumno no sabe acceder o no puede entrar al campus virtual.', respuesta_texto: 'Para ingresar al campus debes seguir los pasos que estan en la imagen: [https://imgur.com/a/ZOtRcpk]', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 9 },
  { nombre: 'ClaveDelCampus', nombre_display: 'Clave del Campus', tipo: 'respuesta_fija', descripcion: 'Proporciona informacion para cuando el alumno no sabe cual es su contrasena o usuario para acceder al campus virtual.', respuesta_texto: 'Tu clave y usuario del campus virtual es con tu nombre y rut. Para entrar sigue los pasos de la siguiente imagen: [https://imgur.com/a/ZOtRcpk]', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 10 },
  { nombre: 'CuotasPendientes', nombre_display: 'Cuotas Pendientes', tipo: 'respuesta_fija', descripcion: 'Proporciona informacion para cuando el usuario no sabe cuanto debe pagar ni como pagar el preuniversitario.', respuesta_texto: 'Para revisar o pagar tus cuotas pendientes: 1. Ingresa a preucv.cl 2. Haz clic en Pago Express 3. Ingresa el RUT del apoderado 4. Revisa el valor total a pagar 5. Realiza el pago a traves de WebPay. Imagen: [https://imgur.com/a/pago-express-preucv-GyBE4IL]', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 11 },
  { nombre: 'paseDeAgendamiento', nombre_display: 'Pase de Agendamiento', tipo: 'respuesta_fija', descripcion: 'Proporciona informacion para cuando los alumnos que van a clases presenciales no saben sacar ni como usar su pase de agendamiento.', respuesta_texto: 'Tu Pase de Agendamiento te lo enviaremos por correo. Llega 30 minutos antes y ten tu pase listo. El codigo QR del pase es tu asistencia. Sin pase no podras ingresar. Imagen: [https://imgur.com/a/G3DgDsb]', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 12 },
  { nombre: 'cambioDeSedeAcademica', nombre_display: 'Cambio de Sede', tipo: 'respuesta_fija', descripcion: 'Proporciona informacion de como solicitar el cambio de sede academica.', respuesta_texto: 'Para solicitar cambiar sede academica debes enviar un correo de manera formal con la solicitud a docencia@pre-ucv.cl', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 13 },
  { nombre: 'cambioDeAsignatura', nombre_display: 'Cambio de Asignatura', tipo: 'respuesta_fija', descripcion: 'Proporciona informacion de como solicitar el cambio de asignatura.', respuesta_texto: 'Para solicitar cambio de asignatura debe enviar un correo de manera formal con la solicitud a docencia@pre-ucv.cl', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 14 },
  { nombre: 'inicioClases', nombre_display: 'Inicio de Clases', tipo: 'respuesta_fija', descripcion: 'Proporciona la informacion de inicio de clases e invoca el tutorial de agendamiento.', respuesta_texto: 'Las actividades de inmersion comienzan 48 horas despues de realizar la matricula, y las clases como tal comienzan el 7 de abril. Toda la informacion le llegara mediante correo electronico.', parametros_openai: { type: 'object', properties: {}, required: [], additionalProperties: false }, orden: 15 },
]

export async function obtenerChatAcademicoConfig(idMarca) {
  try {
    const { data, error } = await supabase
    ...
    console.error('Error en obtenerChatAcademicoConfig:', error)
    return { success: false, error: error.message }
  }
}

export async function crearChatAcademicoConfig(idMarca, datos = {}, nombreUsuario = '') {
  try {
    const { data, error } = await supabase
      .from('chat_academico_config')
      .insert([{
        id_marca: idMarca,
        prompt_rol: datos.prompt_rol || '',
        prompt_estilo: datos.prompt_estilo || '',
        prompt_reglas: datos.prompt_reglas || '',
        prompt_consideraciones: datos.prompt_consideraciones || '',
    ...
    // Seed herramientas iniciales
    const semillas = HERRAMIENTAS_SEMILLA.map(h => ({
      ...h,
      id_marca: idMarca,
      es_semilla: true,
      actualizado_por: nombreUsuario
    ...
    const { error: errorSeed } = await supabase
      .from('chat_academico_herramientas')
      .insert(semillas)

    if (errorSeed) console.error('Error seeding herramientas:', errorSeed)

    return { success: true, data }
  } catch (error) {
    console.error('Error en crearChatAcademicoConfig:', error)
    return { success: false, error: error.message }
  }
}

export async function actualizarChatAcademicoConfig(idMarca, datos, nombreUsuario = '') {
  try {
    const camposPermitidos = [
      'prompt_rol', 'prompt_estilo', 'prompt_reglas', 'prompt_consideraciones', 'prompt_actividades',
      'prompt_clasificacion', 'prompt_contexto_cliente',
      'tiempo_espera_respuesta', 'intentos_reactivacion',
      'mensaje_reactivacion', 'mensaje_despedida', 'mensaje_timeout', 'mensaje_error', 'mensaje_derivacion',
      'webhook_derivacion', 'webhook_callback',
      'temperatura', 'max_tokens', 'parallel_tool_calls',
      'canales_activos', 'max_mensajes_conversacion', 'max_tokens_contexto', 'max_iteraciones_tools',
      'palabras_derivar', 'palabras_urgente', 'buffer_espera_segundos'
    ]

    const update = { actualizado_en: new Date().toISOString(), actualizado_por: nombreUsuario }
    for (const campo of camposPermitidos) {
      if (datos[campo] !== undefined) {
        update[campo] = datos[campo]
      }
    ...
    console.error('Error en actualizarChatAcademicoConfig:', error)
    return { success: false, error: error.message }
  }
}

export async function obtenerChatAcademicoHerramientas(idMarca) {
  try {
    const { data, error } = await supabase
      .from('chat_academico_herramientas')
      .select('*')
    ...
    console.error('Error en obtenerChatAcademicoHerramientas:', error)
    return { success: false, error: error.message }
  }
}

export async function crearChatAcademicoHerramienta(idMarca, datos, nombreUsuario = '') {
  try {
    const { data, error } = await supabase
      .from('chat_academico_herramientas')
      .insert([{
        id_marca: idMarca,
        nombre: datos.nombre,
        nombre_display: datos.nombre_display || datos.nombre,
        descripcion: datos.descripcion,
        tipo: datos.tipo || 'respuesta_fija',
        respuesta_texto: datos.respuesta_texto || '',
        id_flujo: datos.id_flujo || null,
        parametros_openai: datos.parametros_openai || { type: 'object', properties: {}, required: [], additionalProperties: false },
        activo: true,
        es_semilla: false,
    ...
    console.error('Error en crearChatAcademicoHerramienta:', error)
    return { success: false, error: error.message }
  }
}

export async function actualizarChatAcademicoHerramienta(id, idMarca, datos, nombreUsuario = '') {
  try {
    const camposPermitidos = [
      'nombre_display', 'descripcion', 'respuesta_texto', 'activo',
      'id_flujo', 'parametros_openai', 'orden', 'nombre'
    ]
    ...
    console.error('Error en actualizarChatAcademicoHerramienta:', error)
    return { success: false, error: error.message }
  }
}

export async function eliminarChatAcademicoHerramienta(id, idMarca) {
  try {
    // No permitir eliminar herramientas semilla
    ...
    if (tool?.es_semilla) {
      return { success: false, error: 'No se puede eliminar una herramienta predefinida. Solo puedes desactivarla.' }
    }
    ...
    console.error('Error en eliminarChatAcademicoHerramienta:', error)
    return { success: false, error: error.message }
  }
}

export async function obtenerChatAcademicoHistorial(idMarca, limit = 50) {
  try {
    const { data, error } = await supabase
      .from('chat_academico_historial')
      .select('*')
      .eq('id_marca', idMarca)
      .order('creado_en', { ascending: false })
      .limit(limit)
    ...
    console.error('Error en obtenerChatAcademicoHistorial:', error)
    return { success: false, error: error.message }
  }
}

export async function registrarChatAcademicoCambio(idMarca, nombreUsuario, seccion, accion, detalle = {}) {
  try {
    await supabase
      .from('chat_academico_historial')
      .insert([{
    ...
    console.error('Error registrando cambio chat academico:', error)
    return { success: false }
  }
}

// ============================================
// CHAT NUMANCIA - CRUD de configuracion, herramientas e historial
// Replica estructural de chatAcademico*. Backend bot corre en Vercel aparte.
// ============================================

export async function obtenerChatNumanciaConfig(idMarca) {
  try {
    const { data, error } = await supabase
    ...
    console.error('Error en obtenerChatNumanciaConfig:', error)
    return { success: false, error: error.message }
  }
}

export async function crearChatNumanciaConfig(idMarca, datos = {}, nombreUsuario = '') {
  try {
    const { data, error } = await supabase
      .from('chat_numancia_config')
      .insert([{
    ...
    console.error('Error en crearChatNumanciaConfig:', error)
    return { success: false, error: error.message }
  }
}

export async function actualizarChatNumanciaConfig(idMarca, datos, nombreUsuario = '') {
  try {
    const camposPermitidos = [
      'prompt_rol', 'prompt_estilo', 'prompt_reglas', 'prompt_consideraciones',
      'tiempo_espera_respuesta', 'intentos_reactivacion',
      'mensaje_bienvenida', 'mensaje_reactivacion', 'mensaje_despedida',
      'mensaje_timeout', 'mensaje_error',
      'webhook_derivacion', 'webhook_callback',
      'modelo_ia', 'temperatura', 'max_tokens', 'parallel_tool_calls',
      'max_mensajes_conversacion', 'max_tokens_contexto', 'max_iteraciones_tools',
      'canales_activos', 'usar_base_cuentas', 'usar_emojis',
      'widget_color_primario', 'widget_color_texto_header', 'widget_posicion',
      'widget_tamano', 'widget_titulo', 'widget_logo_url', 'widget_activo',
      // campos avanzados configurables (anteriormente hardcodeados)
      'prompt_senales_cierre', 'prompt_resumen', 'regla_sin_emojis',
      'nota_encabezado_cierre', 'nota_encabezado_derivar',
      'mensaje_fallback', 'permitir_derivacion'
    ]

    const update = { actualizado_en: new Date().toISOString(), actualizado_por: nombreUsuario }
    for (const campo of camposPermitidos) {
      if (datos[campo] !== undefined) update[campo] = datos[campo]
    }
    ...
    console.error('Error en actualizarChatNumanciaConfig:', error)
    return { success: false, error: error.message }
  }
}

export async function regenerarChatNumanciaApiKey(idMarca) {
  try {
    const { data, error } = await supabase
      .from('chat_numancia_config')
      .update({ api_key: crypto.randomUUID(), actualizado_en: new Date().toISOString() })
      .eq('id_marca', idMarca)
      .select('api_key')
      .single()
    ...
    console.error('Error en regenerarChatNumanciaApiKey:', error)
    return { success: false, error: error.message }
  }
}

export async function obtenerChatNumanciaHerramientas(idMarca) {
  try {
    const { data, error } = await supabase
    ...
    console.error('Error en obtenerChatNumanciaHerramientas:', error)
    return { success: false, error: error.message }
  }
}

export async function crearChatNumanciaHerramienta(idMarca, datos, nombreUsuario = '') {
  try {
    const { data, error } = await supabase
      .from('chat_numancia_herramientas')
      .insert([{
        ...
        tipo: datos.tipo || 'respuesta_fija',
        respuesta_texto: datos.respuesta_texto || '',
        base_cuentas_filtro: datos.base_cuentas_filtro || {},
        id_flujo: datos.id_flujo || null,
        parametros_openai: datos.parametros_openai || { type: 'object', properties: {}, required: [], additionalProperties: false },
        activo: true,
        es_semilla: false,
    ...
    console.error('Error en crearChatNumanciaHerramienta:', error)
    return { success: false, error: error.message }
  }
}

export async function actualizarChatNumanciaHerramienta(id, idMarca, datos, nombreUsuario = '') {
  try {
    const camposPermitidos = [
      'nombre_display', 'descripcion', 'respuesta_texto', 'activo',
      'id_flujo', 'parametros_openai', 'base_cuentas_filtro', 'orden', 'nombre', 'tipo'
    ]
    ...
    console.error('Error en actualizarChatNumanciaHerramienta:', error)
    return { success: false, error: error.message }
  }
}

export async function eliminarChatNumanciaHerramienta(id, idMarca) {
  try {
    const { data: tool } = await supabase
      .from('chat_numancia_herramientas')
      .select('es_semilla')
    ...
    console.error('Error en eliminarChatNumanciaHerramienta:', error)
    return { success: false, error: error.message }
  }
}

export async function obtenerChatNumanciaHistorial(idMarca, limit = 50) {
  try {
    const { data, error } = await supabase
      .from('chat_numancia_historial')
      .select('*')
      .eq('id_marca', idMarca)
      .order('creado_en', { ascending: false })
      .limit(limit)
    ...
    console.error('Error en obtenerChatNumanciaHistorial:', error)
    return { success: false, error: error.message }
  }
}

export async function registrarChatNumanciaCambio(idMarca, nombreUsuario, seccion, accion, detalle = {}) {
  try {
    await supabase
      .from('chat_numancia_historial')
      .insert([{ id_marca: idMarca, usuario_nombre: nombreUsuario, seccion, accion, detalle }])
    return { success: true }
  } catch (error) {
    console.error('Error registrando cambio chat numancia:', error)
    return { success: false }
  }
}
