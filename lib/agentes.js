  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        restricciones: datos.restricciones || null,
        formato_respuesta: datos.formato_respuesta || null,
        ejemplos: datos.ejemplos || null,
        idioma: datos.idioma || null,
        mensaje_bienvenida: datos.mensaje_bienvenida || null,
        mensaje_despedida: datos.mensaje_despedida || null,
        mensaje_error: datos.mensaje_error || null,
        mensaje_fuera_tema: datos.mensaje_fuera_tema || null,
        mensaje_transferencia: datos.mensaje_transferencia || null,
        mensaje_limite_turnos: datos.mensaje_limite_turnos || null,
        max_tokens: datos.max_tokens || 800,
        max_historial: datos.max_historial || 20,
        max_conocimiento: datos.max_conocimiento || 15,
        prompt_sistema_custom: datos.prompt_sistema_custom || null,
        // Nuevos campos sin hardcode (ver migracion_agente_sin_hardcode.sql)
        instrucciones_cierre: datos.instrucciones_cierre || null,
        plantilla_salidas: datos.plantilla_salidas || null,
        label_variables: datos.label_variables || null
      })
      .select()
      .single()
    ...
    // Agente creado vacío. Las herramientas del catálogo se habilitan
    // explícitamente desde el panel — no se siembran defaults para evitar
...
export async function actualizarAgente(id, datos) {
  try {
    ...
    // Nuevos campos configurables
    if (datos.personalidad !== undefined) campos.personalidad = datos.personalidad
    if (datos.reglas !== undefined) campos.reglas = datos.reglas
    if (datos.restricciones !== undefined) campos.restricciones = datos.restricciones
    if (datos.formato_respuesta !== undefined) campos.formato_respuesta = datos.formato_respuesta
    if (datos.ejemplos !== undefined) campos.ejemplos = datos.ejemplos
    if (datos.idioma !== undefined) campos.idioma = datos.idioma
    if (datos.mensaje_bienvenida !== undefined) campos.mensaje_bienvenida = datos.mensaje_bienvenida
    if (datos.mensaje_despedida !== undefined) campos.mensaje_despedida = datos.mensaje_despedida
    if (datos.mensaje_error !== undefined) campos.mensaje_error = datos.mensaje_error
    if (datos.mensaje_fuera_tema !== undefined) campos.mensaje_fuera_tema = datos.mensaje_fuera_tema
    if (datos.mensaje_transferencia !== undefined) campos.mensaje_transferencia = datos.mensaje_transferencia
    if (datos.mensaje_limite_turnos !== undefined) campos.mensaje_limite_turnos = datos.mensaje_limite_turnos
    if (datos.max_tokens !== undefined) campos.max_tokens = datos.max_tokens
    if (datos.max_historial !== undefined) campos.max_historial = datos.max_historial
    if (datos.max_conocimiento !== undefined) campos.max_conocimiento = datos.max_conocimiento
    if (datos.prompt_sistema_custom !== undefined) campos.prompt_sistema_custom = datos.prompt_sistema_custom
    if (datos.instrucciones_cierre !== undefined) campos.instrucciones_cierre = datos.instrucciones_cierre
    if (datos.plantilla_salidas !== undefined) campos.plantilla_salidas = datos.plantilla_salidas
    if (datos.label_variables !== undefined) campos.label_variables = datos.label_variables
