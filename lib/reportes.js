...
export async function exportarPDF(datos, columnas, titulo = 'Reporte', nombreMarca = '') {
  return new Promise((resolve, reject) => {
    try {
      ...
      y += rowHeight
      doc.moveTo(startX, y).lineTo(startX + pageWidth, y).stroke('#cccccc')
      y += 4
      ...
      if (datos.length > maxRows) {
        doc.moveDown(1)
        doc.fontSize(8).text(`... y ${datos.length - maxRows} registros mas`, { align: 'center' })
      }
